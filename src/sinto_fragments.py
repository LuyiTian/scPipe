...
def fragments(
    ...
    p = Pool(nproc)
	# iterable list of all getFragments(chrom.items)

	
    frag_lists = [
		# map_async takes an iterable and applys the first arg to every element.
		# the iterable is split into chunks and all chunks are run in parallel
        p.map_async(
			# so basiclaly this is
			# map_async(getFragments, list(chrom.items()))
			# functools.partial creates a function equivalent to getFragments(**filled arguments)
            # don't need to worry about it, just retype function
			functools.partial(
                getFragments,
                bam=bam,
	...
	# get filenames from async result. .get() gives return from getFragments, which is filename
