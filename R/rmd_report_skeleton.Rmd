...
# params:
#   log_and_stats_folder: /stornext/Home/data/allstaff/y/yang.p/repos/final_repos/scPipe/scPipe-atac-output/scPipe_atac_stats
editor_options: 
...
```{r}
stats_file <- paste0(params$log_and_stats_folder, "/mapping_stats_per_barcode.csv")
raw <- readr::read_csv(stats_file)
...
```{r demux-stats-table, fig.wide = TRUE, fig.cap = sprintf("Demultiplexing statistics table"), fig.asp = 0.9}
DT::datatable(filtered, options=list(paging=FALSE, searching=FALSE))
```

```{r demux-stats-graph, fig.wide = TRUE, fig.cap = sprintf("Demultiplexing statistics graph"), fig.asp = 0.9}
filtered$count <- filtered$count / sum(filtered$count)
ggplot2::ggplot(filtered, ggplot2::aes(x=status, y=count, fill=status)) +
  ggplot2::geom_col() +
...
```{r barcode-counts-dist, fig.wide = TRUE, fig.cap = sprintf("Read count per barcode"), fig.asp = 0.9}

n_reads_barcode_filename    <- paste0(params$log_and_stats_folder, "/mapping_stats_per_barcode.csv")
...
number_of_reads_per_barcode %>% 
  ...
  ggplot(aes(x=reorder(barcode, number_of_reads), y=log2(number_of_reads))) +
    geom_bar(stat="identity") + xlab("barcode") + ylab("log_2 number_of_reads") +
...
```{r percell-counts, fig.wide = TRUE, fig.cap = sprintf("Counts per cell"), fig.asp = 0.9}
...
```{r percell-features, fig.wide = TRUE, fig.cap = sprintf("Features per cell"), fig.asp = 0.9}
...
```{r perfeature-counts, fig.wide = TRUE, fig.cap = sprintf("Counts per feature"), fig.asp = 0.9}
...
```{r perfeature-cells, fig.wide = TRUE, fig.cap = sprintf("Cells per feature"), fig.asp = 0.9}
...
```{r corr-cells, fig.wide = TRUE, fig.cap = sprintf("Counts vs features per cell"), fig.asp = 0.9}
...
```{r corr-features, fig.wide = TRUE, fig.cap = sprintf("Counts vs cells per feature"), fig.asp = 0.9}
...
```{r  frag-peak, fig.wide = TRUE, fig.cap = sprintf("Total fragments vs fraction in peaks"), fig.asp = 0.9}
...
```{r insert-size-dist, fig.wide = TRUE, fig.cap = sprintf("Distribution of Insert Size"), fig.asp = 0.9}
  frags[, 'isize' := V3 - V2]
...
```{r frag-density, fig.wide = TRUE, fig.cap = sprintf("Density plot of total number of unique fragments"), fig.asp = 0.9}
bc_stat[, 'group' := ifelse(bc %in% cell_barcodes, 'cell', 'non-cell')]
...
```{r anno-overlap-boxes, fig.wide = TRUE, fig.cap = sprintf("Box plots of overlap with annotated regions"), fig.asp = 0.9}
...
```{r overall-overlap, fig.wide = TRUE, fig.cap = "Overlap of fragments with annotated regions", fig.asp = 0.9}
