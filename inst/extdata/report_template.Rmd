...
```{r}
if(params$species != "SPECIES__" & params$gene_id_type != "GENE_ID_TYPE__"){
  scd = create_scd_by_dir(params$outdir, organism=params$species, gene_id_type=params$gene_id_type)
}else{
  scd = create_scd_by_dir(params$outdir)
}
...
if(!all(colSums(pData(QCMetrics(scd)))>0)){
  QCMetrics(scd) = pData(QCMetrics(scd))[,colSums(pData(QCMetrics(scd)))>0]
}
datatable(pData(QCMetrics(scd)),width=800,options=list(scrollX= TRUE))
```

summary of all QC metrics:
```{r}
datatable(do.call(cbind, lapply(pData(QCMetrics(scd)), summary)),width=800,options=list(scrollX= TRUE))
```

Number of reads mapped to exon before UMI deduplication VS number of genes detected:
```{r}
ggplotly(ggplot(pData(QCMetrics(scd)), aes(x=mapped_to_exon,y=number_of_genes))+geom_point(alpha=0.8))
```
...
table(pData(QCMetrics(scd_qc))$outliers)
```
