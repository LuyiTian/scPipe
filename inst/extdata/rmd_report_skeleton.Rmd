---
title: 'scPipe_atac report `r params$organism` scPipe `r package.version("scPipe")`'
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
```{r}
# trimbarcode_stats_filename    <- file.path("/stornext/Home/data/allstaff/y/yang.p/repos/final_repos/scPipe/output", "scPipe_atac_stats", "stats_file_trimbarcode.txt")
...
alignment_stats          <- readr::read_csv(alignment_stats_filename, col_names = FALSE, skip = 1) %>% as.data.frame()
...
```{r}
alignment_stats$count <- alignment_stats$X2 / sum(alignment_stats$X2)
colnames(alignment_stats)[1] <- "Statistic"
ggplot2::ggplot(alignment_stats, ggplot2::aes(x=Statistic, y=count, fill=Statistic)) +
...
ggplot2::ggplot(number_of_reads_per_barcode, 
...
```{r}
cell_stats    <- readr::read_csv(file.path(params$input_folder, "scPipe_atac_stats", "filtered_stats_per_cell.csv"))
```
...
```{r}
feature_stats <- readr::read_csv(file.path(params$input_folder, "scPipe_atac_stats", "filtered_stats_per_feature.csv"))
```
