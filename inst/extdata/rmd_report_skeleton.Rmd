...
params:
  ...
  feature_file: "FEATURE_FILE__"
  n_barcode_subset: "N_BARCODE_SUBSET__"
  tss_file: "TSS_FILE__"
  promoter_file: "PROMOTER_FILE__"
  enhancer_file: "ENHANCER_FILE__"
editor_options:
...
peaks_file                     <- file.path(params$feature_file)
promoters_file                 <- file.path(params$promoter_file)
tss_file                       <- file.path(params$tss_file)
enhs_file                      <- file.path(params$enhancer_file)
...
g             <- plotly::ggplotly(ggplot(data = bc_stat_down, 
...
p     <- plotly::ggplotly(ggplot(data = bc_stat, aes(x = total_frags, fill = group)) + 
                            geom_density() + 
...
plotly::ggplotly(ggplot(data = qc_sele_df, aes(y = frac, x = type, fill = type)) + ylab('Fraction') + theme_bw() +
