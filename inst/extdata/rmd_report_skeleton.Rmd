...
A plot showing the read count distribution across chromosomes is shown below. Note that the normalised metric used is TPM (transcripts per million), which also takes into account of chromosome sizes (also reflected in the bar widths).
...
Shows the alignment statistics for reads with a matched/unmatched barcodes. Note that if a fastq file with the barcodes was supplied then there will be no unmatched barcodes (they can only exist a csv file with barcodes was supplied).
